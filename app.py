...
# ==================== 数据加载（无 widget 调用，纯数据逻辑） ====================
@st.cache_data
def load_data():
    """
    自动查找并读取所有匹配的 CSV 文件，返回合并后的 DataFrame。
    不包含任何 st.sidebar 调用，以避免 CachedWidgetWarning。
    """
    # 获取当前目录下所有 CSV 文件
    ...
    # 自动匹配：文件名包含 "pollutants" 和 "hourly"（不区分大小写）
    ...
    for file in matched_files:
        ...
        for enc in ['gb18030', 'utf-8', 'gbk', 'latin1']:
            try:
                ...
                loaded_files_info.append(f"{file} (编码: {enc})")
                success = True
                break
                ...
                continue
        if not success:
            loaded_files_info.append(f"{file} (解码失败)")
    
    if not df_list:
        return None, matched_files, "未能成功读取任何数据文件，请检查文件格式或编码。"
    ...
    if missing_cols:
        return None, matched_files, f"数据文件缺少必要列: {missing_cols}。现有列: {list(df.columns)}"
    
    # 构造 datetime 列
    df['datetime'] = pd.to_datetime(df['date'].astype(str) + ' ' + df['hour'].astype(str) + ':00:00')
    df = df.sort_values(['station', 'datetime']).reset_index(drop=True)
    return df, loaded_files_info, None  # 第三个元素为 None 表示无错误

# 加载数据
df, loaded_files, error_msg = load_data()

# ==================== 显示加载结果（在缓存函数外部） ====================
if error_msg:
    st.error(f"❌ {error_msg}")
    if df is None and 'loaded_files' in locals() and loaded_files:
...
with st.expander("📄 查看当前站点数据预览"):
    preview_df = df_hour.reset_index()[['datetime', 'AQI', 'PM2.5', 'O3']].head(20)
